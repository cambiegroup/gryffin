                        ...
                        for element in db_content[key]["parameters"][dict_key]:
                            if "category_details" in element:
                                db_clean[key]["parameters"][dict_key].append(
                                    {"category_details": element["category_details"]}
                                )
                        continue
                    db_clean[key]["parameters"][dict_key] = db_content[key][
                        "parameters"
                    ][dict_key]
            ...
            if key in ["descriptor_summary"]:
                ...
                for feature_index, feature_name in enumerate(self.config.feature_names):
                    ...
                    for key, value in sum_dict.items():
                        sum_dict[key] = {
                            inner_key: value[inner_key].tolist()
                ...
                if isinstance(first_element, np.ndarray):
                    ...
                    for element in values:
                        if len(element.shape) == 0:
                            ...
                            new_values.append(
                                [atom_element.item() for atom_element in element]
                            )
                else:
                    new_values = values
            else:
