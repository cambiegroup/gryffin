...
class SqliteDatabase(Logger):
    ...
    def _return_dict(function):
        def wrapper(self, *args, **kwargs):
            ...
            info_dicts = [
                {key: entry[key] for key in self.attributes} for entry in entries
            ]
            return info_dicts
    ...
    def _process_requests(self):
        ...
        while keep_processing:
            ...
            keep_processing = (
                len(self.WRITING_REQUESTS) > 0
                or len(self.UPDATE_REQUESTS) > 0
                or len(self.READING_REQUESTS) > 0
            )
        ...
        for index, key in enumerate(condition_keys):
            if isinstance(condition_values[index], list):
                ...
                filters = [
                    ...
                    for value in condition_values[index]
                ]
    ...
    def update_all(self, condition_dict, update_dict):
        ...
        for index, key in enumerate(condition_keys[1:]):
            update = update.where(
                getattr(self.table.c, key) == condition_values[index + 1]
            )
